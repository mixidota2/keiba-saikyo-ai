...
- **タイム適性チェック**:
  - タイム適性ファクター（time_performance）が適切に計算されているか
  - 持時計データが不足している馬のスコアが50点に設定されているか
  - 基準タイムの馬場補正が適用されているか
- **投資配分の整合性チェック（過去の教訓）**:
  - 各買い目の `investment_ratio` が `expected_return`（EV）の高い順に概ね対応しているか
  - EV < 100%の組み合わせに配分が割かれていないか
  - 穴目への高配分（全体15%超）の根拠がEV計算であり、Vデータ等の統計適合だけではないか
  - スコア上位2頭のうち少なくとも1頭が買い目の相手に含まれているか（INS-006）
  - 軸馬の騎手力ファクターが50点以上か（INS-007）
  - ペース予想の結論と買い目構成が矛盾していないか（INS-008）
  - 上記に違反がある場合は severity: "medium" 以上として指摘する
  - **注意: 人気馬を含む組み合わせの強制追加や、スコア順位による配分固定は求めない。EVのみで判断すること**
